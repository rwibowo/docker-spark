...
function usage {
  ...
  --endpoint endPoint                                   Set S3 end point
    ...
    case "$1" in
      ...
      --endpoint)
        if [[ -n "$2" ]]; then
          ...
          printf '"--endpoint" requires a non-empty option argument.\n'
          usage 
          exit 1
        fi
      ;;
...
if [[ "$enablePVC" == "true" ]]; then
    ...
    if [[ "$enableIAM" == "false" ]]; then
      export SPARK_HISTORY_OPTS="$SPARK_HISTORY_OPTS \
      -Dspark.hadoop.fs.s3a.access.key=$(cat /run/secrets/${accessKeyName}) \
      -Dspark.hadoop.fs.s3a.secret.key=$(cat /run/secrets/${secretKeyName})";
    fi;
    if [[ -n ${endPoint} ]]; then
      export SPARK_HISTORY_OPTS="$SPARK_HISTORY_OPTS \
...
exec /sbin/tini -s -- $SPARK_HOME/bin/spark-class org.apache.spark.deploy.history.HistoryServer
